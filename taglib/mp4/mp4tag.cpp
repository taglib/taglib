...
MP4::Tag::parseFreeForm(const MP4::Atom *atom)
{
  ...
  if(data.size() > 2) {
    ...
    AtomDataList::ConstIterator begin = it;
    for(; it != data.end(); ++it) {
      if(it->type != type) {
...
{
  MP4::AtomList::ConstIterator end = path.end();
  std::advance(end, -ignore);

  for(MP4::AtomList::ConstIterator it = path.begin(); it != end; ++it) {
