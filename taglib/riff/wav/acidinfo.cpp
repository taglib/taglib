...
ByteVector RIFF::WAV::AcidInfo::render() const
{
  ...
  data.append(ByteVector::fromFloat(d->unknown2));
  ...
  data.append(ByteVector::fromFloat(d->tempo));
...
void RIFF::WAV::AcidInfo::read(const ByteVector &data)
{
  ...
  short rootNote = data.mid(4, 6).toShort(false);
  d->rootNote = (d->flags & RootNote) ? rootNote  : (rootNote - 12);

  d->unknown1 = data.mid(6, 8).toShort(false);
  d->unknown2 = data.mid(8, 12).toFloat();
  d->numberOfBeats = data.mid(12, 16).toUInt(false);
  d->timeSignatureDenominator = data.mid(16, 18).toShort(false);
  d->timeSignatureNumerator = data.mid(18, 20).toShort(false);
  d->tempo = data.mid(20, 24).toFloat();
}
