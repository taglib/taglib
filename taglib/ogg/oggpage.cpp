...
ByteVector Ogg::Page::render() const
{
  ...
  // Compute and set the CRC32 checksum for the Ogg page.  The checksum is taken
  // over the entire page with the 4 bytes reserved for the checksum zeroed and
  // then inserted in bytes 22-25 of the page header.

  const ByteVector checksum = ByteVector::fromUInt(crc32(data), false);
