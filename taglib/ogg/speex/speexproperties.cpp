...
void Speex::Properties::read()
{
  ...
  d->speexVersion = data.toUInt(pos, 4, false);
  pos += 4;
  ...
  d->sampleRate = data.toUInt(pos, 4, false);
  pos += 4;

  // mode;                   /**< Mode used (0 for narrowband, 1 for wideband) */
  d->mode = data.toUInt(pos, 4, false);
  pos += 4;
  ...
  d->channels = data.toUInt(pos, 4, false);
  pos += 4;

  // bitrate;                /**< Bit-rate used */
  d->bitrate = data.toUInt(pos, 4, false);
  pos += 4;
  ...
  d->vbr = data.toUInt(pos, 4, false) == 1;
  pos += 4;
