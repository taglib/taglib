...
void WavPack::Properties::read(File *file, long streamLength, ReadStyle style)
{
  ...
  if(!data.startsWith("wvpk")) {
    debug("WavPack::Properties::read() -- invalid header signature.");
    return;
  }
  ...
  unsigned int samples = data.toUInt(12, false);
  if(samples == ~0u) {
    if(file && style != Fast) {
